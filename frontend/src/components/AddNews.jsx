...
function AddNews() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ❌ REMOVE this header — browser sets it automatically!
      await api.post("/user/addNews", data, {
        withCredentials: true,
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#1A2A4F] px-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {formData.media && (
            <div className="flex justify-center">{getFilePreview()}</div>
          )}
          ...
          <motion.select
            ...
            className="w-full px-4 py-3 border rounded-xl bg-[#FFF2EF]/90"
          >
            {["Politics", "Sports", "Entertainment", "Technology", "Health", "Other"].map(
              (cat) => (
        ...
        {formData.place && (
          <p className="mt-3 text-sm text-black text-center">
            📍 Detected Location: {formData.place}
          </p>
        )}
      </motion.div>
    </div>
